...
- The docs as a site: `make docs` builds the two pages above, one how-to per task
  (write a plugin, add a file format, swap the technology behind a port, watch a turn),
  the written reference — every setting, every port, every component — and a reference
  page for every module of `cora.domain`, `cora.ports`, `cora.engine` and `cora.app`,
  generated from the source by `scripts/gen_reference.py`; `make docs-serve` reads it with live

...
`make diagram` needs nothing but the dev group — `grimp` reads the imports, the api's and the
engine's classes are asked for their own annotations, and one scripted turn reports its trace. A picture that

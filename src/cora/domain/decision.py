...
@dataclass(frozen=True)
class Pending:
    """A turn parked on a decision.

    `asked` is the question the turn opened with, kept beside the decision because a
...
class TurnPaused(Exception):
    ...
    def __init__(self, pending: Pending) -> None:
        """Carry the parked turn; the question doubles as the exception's message."""

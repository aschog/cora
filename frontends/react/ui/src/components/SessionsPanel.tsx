...
export default function SessionsPanel({
  ...
  /* Only one row is ever out: a rail of half-open rows says nothing about which
     conversation a delete belongs to. */
  const start = (thread: string, from: number) => setDrawn({ thread, from, at: 0 })
  ...
  /** Let go: the row snaps to uncovered or shut, whichever the gesture was nearer to
     asking for — and a gesture too small to be one leaves it where it was. */
    ...
    setDrawn((held) =>
      held?.thread !== thread || held.from === null
        ...
        : {
            thread,
            from: null,
            at: held.at <= -A_GESTURE ? -UNCOVERED : 0,
          },
    )
        ...
        return (
          <div key={session.thread_id} className="session-row">
            <div
              ...
              style={{ transform: `translateX(${inUse ? 0 : sitting(session.thread_id)}px)` }}
              onPointerDown={
              ...
              >
                <svg width="15" height="15" viewBox="0 0 20 20" fill="none" aria-hidden="true">
                  <path
                    ...
                    strokeLinecap="round"
                  />
                  <path d="M8.4 8.4v5.6M11.6 8.4v5.6" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" />
                </svg>
              </button>
            )}
          </div>

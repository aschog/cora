...
test("a list is named for the language it is in", async ({ page }) => {
  ...
  const named = await page.evaluate(() => {
    ...
    const lit = { name: "Lietuvių", slug: "lietuviu" };
    return {
      titled: listName(lit, "Einheit 3"),
      /* No title names it for the day it was saved. */
      dated: listName(lit, ""),
    };
  });

  expect(named.titled).toBe("lietuviu-einheit-3.md");
  expect(named.dated).toMatch(/^lietuviu-\d{4}-\d{2}-\d{2}\.md$/);
});
...
test("a reading becomes rows split where the gap is", async ({ page }) => {
  ...
  const read = await page.evaluate(() => {
    ...
    return {
      columns: rowsFrom("Hilfe     pagalba\nHaus\tnamas"),
      dashed: rowsFrom("der Hund — šuo"),
      spaced: rowsFrom("Hilfe pagalba"),
      /* A line the reading gave one word of is offered with the other half empty. */
      lonely: rowsFrom("pagalba"),
      blank: rowsFrom("\n  \n"),
    };
  });
  ...
  expect(read.dashed).toEqual([{ German: "der Hund", learning: "šuo" }]);
  expect(read.spaced).toEqual([{ German: "Hilfe", learning: "pagalba" }]);
  expect(read.lonely).toEqual([{ German: "pagalba", learning: "" }]);
...
test("a corrected row is what the vocab field is given", async ({ page }) => {
  ...
  await row.locator("input").nth(1).fill("pagalba");
  ...
  await expect(page.locator("#said")).toContainText("lietuviu-einheit-3.md");

  const held = await page.request.get(
    "/api/documents/vocab/lietuviu-einheit-3.md",
  );
  expect(held.ok()).toBe(true);
  const [upload] = await held.json();
  expect(upload.text).toContain("| Hilfe | pagalba |");
});
...
test("an upload cora refuses is reported and left to copy", async ({
  ...
  await row.locator("input").nth(1).fill("namas");
  await page.getByRole("button", { name: "Save to cora" }).click();

  await expect(page.locator("#said")).toContainText("refused");
  await expect(page.locator("#saved")).toContainText("| Haus | namas |");
});
...
test("a screenshot read on the page is corrected and saved into the field", async ({
  ...
  await expect(page.locator("#said")).toContainText("lietuviu-einheit-5.md");

  const held = await page.request.get(
    "/api/documents/vocab/lietuviu-einheit-5.md",
  );
  const [upload] = await held.json();

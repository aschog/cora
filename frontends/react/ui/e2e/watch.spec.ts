...
test("a workout finished on the wrist is a document of the fitness field", async ({
  ...
  expect(await held.json()).toEqual(
    expect.arrayContaining([expect.stringMatching(saved)]),
  );
});
...
test("last workout's notice leaves this workout's start alone", async ({
  page,
}) => {
...
test("a field whose notice nobody wrote leaves the trainer as it is", async ({
  page,
}) => {
...
test("a tap on the wrist saves the workout and starts a fresh one", async ({
  page,
}) => {
  await wrist(page, RUNNING);
  await page.goto(TRAINER);
  ...
  await expect(page.locator("#warn")).toContainText("Watch ended the workout");
  await expect(page.locator(".set.on")).toHaveCount(0);
...
test("a field that speaks puts its own conversation on the screen", async ({
  page,
}) => {

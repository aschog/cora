...
import { folded, fresh } from './helpers'
...
const foldedAfresh = async (page: import('@playwright/test').Page, named: RegExp) => {
  await fresh(page)
  return folded(page, named)
}
...
test('the control left behind by a folded rail is drawn in the accent', async ({
  ...
  for (const named of [/Documents/, /Plan & memory/]) {
    const control = await foldedAfresh(page, named)
...
test('the folded left rail keeps the name of the page beside its control', async ({
  ...
  const control = await foldedAfresh(page, /Documents/)

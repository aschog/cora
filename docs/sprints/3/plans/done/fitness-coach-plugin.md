...
- [x] a benign fitness question ("how much protein to build muscle") passes without raising

#### Seed docs

- [x] `seed_docs` is a non-empty tuple of (markdown filename, non-empty UTF-8 bytes) pairs
- [x] every seed doc ingests through `KnowledgeBase.add_file` with fakes, producing ≥ 1 chunk
...
- [x] wired into a `ValidationPipeline`, the plugin's rule runs after the core rules

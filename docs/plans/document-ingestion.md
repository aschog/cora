...
- [x] md bytes load like txt, markup preserved verbatim

**Loaders — pdf** (`uv add pypdf` — pinned `>=6.14,<7` — as part of the first red step below)
- [x] a single-page PDF generated by the in-test fixture helper (pypdf writer API) round-trips its sentence through the PDF loader
- [x] a multi-page PDF yields pages in order, joined by a paragraph break
- [x] blank/image-only pages contribute nothing and raise nothing
- [x] corrupt PDF bytes raise the unreadable-file error (pypdf's exception never escapes)

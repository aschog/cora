...
3. **Add a tool, if it has one.** A tool is a name, a description the model reads, a
   ...
   **A card asks for two values or more.** cora refuses a card of a single field
   before it is put, and tells the model to ask for that value in its answer — so a
   schema with one property gets no card at all, and a call missing one of several
   arguments gets none either. The reader replies in prose and the model calls the tool
   again with what they said. Nothing to write for it: build the card out of the schema
   as above, and the rule holds wherever your card came from.

...
| **Documents** | `keep(name, text)`, `read(name) -> str \| None` | `SqliteDocuments` — one row per document at `CORA_DOCUMENTS_PATH`, holding the *cleaned* text ingestion chunked. A citation is a span of that text, so the pane can open `[n]` and show the passage in place; without it a number would name a document nobody could read. |

...
screen folded, the picture SHALL be the whole screen until the camera closes. The screen
SHALL be taken for the picture rather than for the permission. A picture taken away
SHALL close the camera. The plan SHALL be read from the sheet the page names, and the
page SHALL hand what it produces to cora and to nothing else. Every other address it
reaches SHALL be one written down, so a new one is a change somebody made rather than a
change nobody saw.

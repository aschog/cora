...
- [ ] 1.1 Write the functional test where three trainer-posted workouts, two on one day, list as two dated sessions with their numbers and not the plan, marked `@pytest.mark.xfail(strict=True)`.

## 2. The grammar, read back

- [ ] 2.1 Write a test that a heading ending in kilograms reads as a movement with that name and load.
- [ ] 2.2 Write a test that a heading ending in `bw` or `bw+n` reads as a bodyweight load.
...
- [ ] 2.8 Write a test that a heading in another script reads as any other.

## 3. What the tool lists

- [ ] 3.1 Write a test that every document named for a day is a session dated from its name, oldest first.
- [ ] 3.2 Write a test that two documents of one day are one session, movements in upload order.
- [ ] 3.3 Write a test that a document not named for a day is not a session.

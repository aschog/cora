...
- The fork is put once a turn still, and a form as often as the round budget allows
...
- `README.md`, `docs/happy-path.md` — that cora asks for values it does not hold
- `docs/privacy-and-ethics.md`, `docs/how-to/write-a-plugin.md` — a fourth name of cora's
- Reading what the reader wrote is shared, so a plugin's cards read a blank as this does
- Left alone: `ask_user`, which still settles one fact between values cora found
- Left alone: `Tool.asks`, and what a plugin declares to get a card of its own

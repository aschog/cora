...
def test_a_word_missed_today_is_the_word_put_in_the_next_conversation(
    ...
    _drilling(missing, store).agent.answer("Drill me.", "today", pin=FIELD)
    ...
    _drilling(asking, store).agent.answer("Drill me again.", "tomorrow", pin=FIELD)
...
def _drilling(model: ScriptedChatModel, store: Any) -> Any:
    app = assembled(
        chat_model=model,
    ...
    indexed(app, ("einheit-3.md", LIST.encode()), scope=FIELD)
    return app
